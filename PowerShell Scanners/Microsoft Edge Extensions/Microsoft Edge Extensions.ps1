...
Foreach ( $User in (Get-ChildItem -Directory -Path "$env:SystemDrive\Users") ) {
    ...
    # Get profile folder
    $EdgeDir = $User.FullName + "\AppData\Local\Microsoft\Edge\User Data"
    $PreferencesFile = ""

    if ( Test-Path "$EdgeDir\Default\Preferences" ) {

        $PreferencesFile = "$EdgeDir\Default\Preferences"
        Write-Verbose "Found --- $PreferencesFile"

    }

    if ( Test-Path "$EdgeDir\Profile 1\Preferences" ) {

        $PreferencesFile = "$EdgeDir\Profile 1\Preferences"
        Write-Verbose "Found --- $PreferencesFile"
    ...
    Foreach ( $Extension in $PreferencesJson.extensions.settings.GetEnumerator() ) {
        ...
        $Output = [Ordered]@{
            ...
            User           = [String]  $User
            EdgeVer      = [String]  $PreferencesJson.extensions.last_Edge_version
        }
        Write-Host $Output
